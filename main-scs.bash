...
cp -f system-auth-orig /etc/pam.d/system-auth
...
cp -f password-auth-orig /etc/pam.d/password-auth
...
cp -f audit.rules-orig /etc/audit/audit.rules

cp -p /etc/audit/rules.d/50-system_local.rules /etc/audit/rules.d/50-system_local.rules_$(date +%F)
cp -f 50-system_local.rules-orig /etc/audit/rules.d/50-system_local.rules
...
sed -f sedfile /etc/rsyslog.conf > /etc/rsyslog.conf2  #this is also a bakup
...
df --local -P | awk {'if (NR!=1) print $6'} | xargs -I '{}' find '{}' -xdev -type d \( -perm -0002 -a ! -perm -1000 \) 2>/dev/null >>stickybitnotset
...
df --local -P | awk {'if (NR!=1) print $6'} | xargs -I '{}' find '{}' -xdev -nogroup >>AD.70.1.4.68_unownedfile
echo "AD.70.1.4.67		Ensure no unowned files or directories exist" 
df --local -P | awk {'if (NR!=1) print $6'} | xargs -I '{}' find '{}' -xdev -nouser >> AD.70.1.4.67_ungroupfile
